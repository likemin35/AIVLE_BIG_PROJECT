...
필수 규칙:
1) **가장 중요**: 약관 조항에 법적·재정적 리스크나 소비자에게 불리한 점이 **명백히** 있을 때만 분석 결과를 출력하세요.
...
5) **절대 규칙**: 리스크가 없거나 경미한 조항은 **절대로** 출력하면 안 됩니다. 분석할 가치가 있는 문제점만 보고하세요.
...
아래 입력을 검토해 위 형식으로만 출력하세요. 위 규칙을 철저히 지켜, 문제가 있는 조항에 대해서만 분석을 제공하세요. 리스크가 없는 내용은 출력에서 완전히 제외해야 합니다.
{clause}
{similar_cases}
